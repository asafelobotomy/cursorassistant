...
| Auto-invokable core skills | ≤ 3 |

## Checklist before merge (instruction surfaces)

1. New skill? Default **`disable-model-invocation: true`** unless it must auto-run (`workspaceSearch`, `ciPreflight`, `testing`).
...
`task-triage`, `lifecycleAudit`, `surfaceReview`, `cursorAssistantSetup` — invoke with `/name`; not ambient procedures.
