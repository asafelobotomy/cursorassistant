...
def tool_testing_show_key_commands(arguments: dict) -> dict:
    ...
    if not commands:
        return {"status": "unavailable", "summary": "No key commands were found in README.md.", "commands": []}
...
def tool_testing_list_tests(arguments: dict) -> dict:
    ...
    command = resolve_key_command("Run tests")
    if command is None:
        return build_tool_result(status="unavailable", summary="No Run tests command is declared in README.md.")
...
mcp = FastMCP("cursorWorkspaceTesting")


@mcp.tool()
def testing_show_key_commands() -> ToolResult:
    """List the workspace key commands declared in README.md."""
